namespace Fosol.Site.Api.Authentication;

using System.Security.Claims;
using System.Text;
using Fosol.Site.Api.Options;
using Entities = Fosol.Site.Entities;
using Microsoft.Extensions.Options;
using Fosol.Site.Api.Models.Auth;
using System.IdentityModel.Tokens.Jwt;
using Microsoft.IdentityModel.Tokens;
using Microsoft.AspNetCore.Authentication.JwtBearer;
using Fosol.Site.Dal.Security;
...
{
  #region Variables
  private readonly SiteAuthenticationOptions _options;
  ...
  /// </summary>
  public Authenticator(IOptions<SiteAuthenticationOptions> options, IHashPassword hashPassword, IUserService dbService, IHttpContextAccessor httpContext)
  {
  ...
  public async Task<TokenModel> AuthenticateAsync(Entities.User user)
  {
    ...
    {
        new Claim(ClaimTypes.NameIdentifier, $"{user.Key}", typeof(string).FullName, FosolIssuer.Issuer, FosolIssuer.OriginalIssuer),
        new Claim(FosolClaimTypes.Id, $"{user.Id}", typeof(long).FullName, FosolIssuer.Issuer, FosolIssuer.OriginalIssuer),
        new Claim(FosolClaimTypes.AccessType, Enum.GetName(user.UserType) ?? String.Empty, typeof(string).FullName, FosolIssuer.Issuer, FosolIssuer.OriginalIssuer),
        new Claim(ClaimTypes.Name, $"{user.Username}", typeof(string).FullName, FosolIssuer.Issuer, FosolIssuer.OriginalIssuer),
        new Claim(ClaimTypes.Email, $"{user.Email}", typeof(string).FullName, FosolIssuer.Issuer, FosolIssuer.OriginalIssuer),
        new Claim(FosolClaimTypes.DisplayName, $"{user.DisplayName}", typeof(string).FullName, FosolIssuer.Issuer, FosolIssuer.OriginalIssuer),
        new Claim(ClaimTypes.GivenName, $"{user.FirstName}", typeof(string).FullName, FosolIssuer.Issuer, FosolIssuer.OriginalIssuer),
        new Claim(ClaimTypes.Surname, $"{user.LastName}", typeof(string).FullName, FosolIssuer.Issuer, FosolIssuer.OriginalIssuer),
    };
    refreshClaims.AddRange(_dbService.GetRoleClaims(user.Id).Select(c => new Claim(ClaimTypes.Role, c.Name, typeof(string).FullName, FosolIssuer.Account(c.AccountId), FosolIssuer.OriginalIssuer)));
    refreshClaims.AddRange(user.Claims.Select(c => new Claim(c.Name, c.Value, typeof(string).FullName, FosolIssuer.Account(c.AccountId), FosolIssuer.OriginalIssuer)));
